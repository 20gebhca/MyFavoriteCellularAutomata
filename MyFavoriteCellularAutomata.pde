  ...
  public void show() {
    for (int i = 0; i < cellArray.length; i++) {
      ...
      if (cellArray[i].equals("1")) {  // TODO: replace true with a useful predicate
        fill(#952424);
        noStroke();
        if( getCurrentGenerationNumber()==0){
          fill(0,0,255);
        }
        if( getCurrentGenerationNumber()==1){
          ...
          fill(0);
                ...
                if( getCurrentGenerationNumber()==2){
          fill(0,0,255);
        }
                if( getCurrentGenerationNumber()==3){
          fill(0);
        }
                if( getCurrentGenerationNumber()==4){
          fill(0,0,255);
        }
                if( getCurrentGenerationNumber()==5){
          fill(0);
        }
                if( getCurrentGenerationNumber()==6){
          fill(0,0,255);
        }
                if( getCurrentGenerationNumber()==7){
          fill(0);
        }
                if( getCurrentGenerationNumber()==8){
          fill(0,0,255);
        }
                if( getCurrentGenerationNumber()==9){
          fill(0);
        }
                if( getCurrentGenerationNumber()==10){
          fill(0,0,255);
        }
                if( getCurrentGenerationNumber()==11){
          fill(0);
        }
